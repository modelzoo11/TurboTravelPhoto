...
Page({
	...
	bindPackageTap: function (e) {
		let packageIdx = pageHelper.dataset(e, 'idx');
		this.setData({ packageIdx });
	},
	...
	bindJoinTap: async function (e) {
		...
		let pacakgeIdx = this.data.pacakgeIdx;
		if (pacakgeIdx < 0) return pageHelper.showNoneToast('请先选择套餐');
		...
		let packageIdx = this.data.packageIdx;
		...
		let callback = async () => {
			try {
				...
				await cloudHelper.callCloudSumbit('meet/before_join', params, opts).then(res => {
					wx.navigateTo({
						url: `../join/meet_join?id=${meetId}&timeMark=${timeMark}&packageIdx=${packageIdx}`,
					})
				});
			} catch (ex) {
				console.log(ex);
