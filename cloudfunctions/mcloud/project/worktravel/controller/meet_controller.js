...
class MeetController extends BaseProjectController {
	...
	async getMeetList() {
		...
		for (let k = 0; k < list.length; k++) {
			...
			delete list[k].MEET_DAYS; 
		 
			list[k].MEET_ITEM_MIN_PRICE = Number(dataUtil.fmtMoney(list[k].MEET_ITEM_MIN_PRICE / 100));
	...
	async detailForJoin() {
		...
		if (meet) {
			...
			let itemList = meet.MEET_ITEM_LIST;
	...
	async beforeJoin() {
		...
		let service = new MeetService();
		await service.beforeJoin(this._userId, input.meetId, input.timeMark);
	...
	async prepay() {
		...
		let service = new MeetService();
		return await service.prepay(this._userId, input.meetId, input.packageIdx, input.timeMark, input.formsList);
	}
