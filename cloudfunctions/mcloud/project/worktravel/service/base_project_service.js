...
class BaseProjectService extends BaseService {
	...
	async initSetup() {
		...
		let F = (c) => 'bx_' + c;
		const INSTALL_CL = 'setup_worktravel';
		const COLLECTIONS = ['setup', 'admin', 'log', 'day', 'fav', 'join', 'meet', 'news', 'temp', 'user', 'product', 'pay'];
		const CONST_PIC = '/images/cover.gif';

		const NEWS_CATE = '1=通知公告';
		const MEET_TYPE = '1=摄影师预约,2=拍摄预约';
		const PRODUCT_CATE = '1=汉服,2=藏服,3=苗服,4=蜜月,5=古城,6=其他';
